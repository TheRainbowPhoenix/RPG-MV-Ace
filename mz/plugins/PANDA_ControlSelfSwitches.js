...
// 2025-01-05 Ver.1.2.0 Bug fix and Added Turn ON Function.
 ...
 * 
 * # Set Self Switches
 * Turn ON the Self Switches of specified Map ID and Event ID.
 *  - Map ID      : Input Map ID. (-1 = current map)
 *  - Event ID    : Input Event ID.
 *  - Self Switch : Select the Self Switch from A-D.
 * 
 * [License]
 ...
 * 
 * @command SET
 * @text Set Self Switches
 * @desc Turn ON the Self Switches of specified Map ID and Event ID.
 * 
 * @arg mapID
 * @text Map ID
 * @desc Input Map ID. (-1 = current map)
 * @type number
 * @min -1
 * @default -1
 * 
 ...
 * @arg switchID
 * @text Self Switch
 * @desc Select the Self Switch from A-D.
 * @type select
 * @option A
 * @option B
 * @option C
 ...
 * 
 * ◆ セルフスイッチのON
 * 指定したマップID、イベントIDのセルフスイッチをONにします。
 * - マップID　：対象のマップIDを指定します。
 *   　　　　　　-1を指定すると現在のマップが対象になります。
 * - イベントID：対象のイベントIDを指定します。
 * - スイッチID：対象のセルフスイッチ(A,B,C,D)を指定します。
 * 
 ...
 * @command SET
 * @text セルフスイッチのON
 * @desc 指定したマップID、イベントIDのセルフスイッチをONにします。
 * 
 * @arg mapID
 * @text マップID
 * @desc マップIDを指定します。-1を指定すると現在のマップが対象になります。
 * @type number
 * @min -1
 * @default -1
 * 
 ...
 * 
 * # 셀프스위치 ON
 * 지정된 맵ID, 이벤트ID에 해당되는 셀프스위치를 ON으로 합니다.
 * - 맵ID     : 대상인 맵ID를 지정합니다.
 *              -1을 지정하면 현재 맵이 대상이 됩니다.
 * - 이벤트ID : 대상인 이벤트ID를 지정합니다.
 * - 스위치ID : 대상인 셀프스위치(A,B,C,D)를 지정합니다.
 ...
 * @command SET
 * @text 셀프스위치 ON
 * @desc 지정된 맵ID, 이벤트ID에 해당되는 셀프스위치를 ON으로 합니다.
 * 
 * @arg mapID
 * @text 맵ID
 * @desc 맵ID를 지정합니다. -1을 지정하면 현재 맵이 대상이 됩니다.
 * @type number
 * @min -1
 * @default -1
 * 
...
(() => {
	...
	PluginManager.registerCommand(pluginName, 'RESET', function(args) {
		...
		// get arguments
		let mapID = parseInt(args['mapID']) || 0;
	...
	//--------------------------------------------------
	// Plugin Command "Set Self Switches"
	//--------------------------------------------------
	PluginManager.registerCommand(pluginName, 'SET', function(args) {
		
		// get arguments
		let mapID = parseInt(args['mapID']) || -1;
		let eventID = parseInt(args['eventID']) || 0;
		let switchID = args['switchID'] || '';
